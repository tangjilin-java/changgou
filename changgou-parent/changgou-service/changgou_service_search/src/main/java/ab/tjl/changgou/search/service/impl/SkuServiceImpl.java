package ab.tjl.changgou.search.service.impl;

import ab.tjl.changgou.goods.feign.SkuFeign;
import ab.tjl.changgou.goods.pojo.Sku;
import ab.tjl.changgou.search.dao.SkuEsMapper;
...
public class SkuServiceImpl implements SkuService {
    ...
    public void importData() {
        ...
        //将skulist转成List<SkuInfo>
        List<SkuInfo> skuInfoList = JSON.parseArray(JSON.toJSONString(skuResult.getData()),SkuInfo.class);
        for (SkuInfo skuInfo : skuInfoList) {
            //获取spec -》MaP(String)->map类型
            Map<String ,Object> specMap = JSON.parseObject(skuInfo.getSpec(),Map.class);
            //如果需要生成动态的域，只需要将该域存入一个Map<String,Object>对象中即可，该map的key会生成一个域 域的名字为该map的key
